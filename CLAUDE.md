...
The biology learning loop (see `docs/PRODUCT-VISION.md`) lives in `src/lib/learning.ts` (attempt log, Error Memory, readiness), `src/lib/planner.ts` (Today's priority formula), `src/data/biologyUnderstanding.ts` (Teach-Back key ideas and Why Ladders) and `src/features/` (`TodayPlan`, `TeachBack`, `WhyLadder`, `CaseLab`, `ErrorMemory`, `Ready`, `MemoryGarden`). The Today plan lives on the Review tab; Home keeps only a slim link to it, because the owner didn't want Home to open with plan chips. Case Lab content is in `src/data/caseLab.ts`. Notes → Quiz is `src/features/NotesQuiz.tsx`: the on-device builder is `src/lib/noteQuiz.ts`, and Plus AI quizzes come from the AI gateway's `quiz` task, which drops questions whose quote isn't in the pasted notes. The Korean course's Hangul Lab is `src/features/HangulLab.tsx`, with jamo tables, Unicode composition and romanization in `src/data/hangul.ts`. The shared Korean audio/recording component is `src/components/VoicePractice.tsx`. Keep the course's existing button labels, which `tests/e2e/new-courses.spec.ts` depends on. On Ready, the vocabulary journal comes first and the biology meters and Memory Garden follow as matching white-paper sections (`s.paper`). All Plus AI goes through one Supabase Edge Function, `ai` (see `docs/AI-ARCHITECTURE.md`). It has four tasks: `tutor`, `quiz`, `story` and `hook`. Prompts, schemas and output checks are in `supabase/functions/_shared/tasks.ts` (pure, unit-tested); the client is `src/lib/ai.ts`. It uses the official Anthropic SDK, `claude-opus-5` by default, with the RevenueCat entitlement checked server-side and per-task quotas in `consume_ai_quota`. Tutor feedback is advice only; the on-device keyword check still decides mastery. On web, purchases use RevenueCat Billing (`EXPO_PUBLIC_REVENUECAT_WEB_KEY`) and require sign-in first. `docs/GO-LIVE.md` and `npm run setup:live` cover the owner's setup; never write keys yourself. Keep measurement honest: attempts that used a hint never count as mastered, and keyword checks are labelled as keyword checks. Motion uses Reanimated and Moti through the `Reveal`, `Pulse`, `PressableScale` and `Meter` components in `src/components/Reveal.tsx`, and each of them respects reduced motion. The logo has no punctuation dot: `src/components/QuasarMark.tsx` matches `assets/icon.svg`; `assets/icon.png` is generated from that SVG with `node scripts/render-icon.cjs`.
