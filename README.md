...
[Open the live demo](https://quasar-memory-garden.reyaattri4.chatgpt.site/) · [Product vision](docs/PRODUCT-VISION.md) · [AI architecture](docs/AI-ARCHITECTURE.md) · [Go live](docs/GO-LIVE.md) · [Design narrative](docs/DESIGN-NARRATIVE.md) · [Release checklist](docs/RELEASE-CHECKLIST.md)
...
| **Memory Worlds** | **Cell City**, a six-episode story world for cells and cellular respiration: a power shortage to investigate from glycolysis to ATP synthase, six recurring characters, the real biology stated before each city cue, rebuild-from-memory games, district readings to reason from, and cues that fade as you answer without them (and return after a miss). Every clue joins spaced review. Also walkable dojo, ruins and neon palaces; illustrated biology, SAT, calculus, Korean and π courses. The Korean course adds a **Hangul Lab**: how the vowels are drawn, a syllable-block builder with batchim, look-alike sound drills (ㄱ/ㅋ/ㄲ, ㅓ/ㅗ…) that remember your mix-ups, and 20 real words to read. | `src/features/CellCity.tsx`, `src/data/cellCity.ts`, `src/features/MemoryPalace.tsx`, `src/features/HangulLab.tsx` and the course folders |
| **Teach-Back** | The lesson hides and you explain it, by typing or, in supporting browsers, by speaking. Missing ideas come back as a *why* question, not the answer. Then you defend it with a follow-up question. Quasar Plus adds **AI tutor feedback** on how you explained it. | `src/features/TeachBack.tsx`, AI task `tutor` |
...
| **Error Memory** | Miss a concept twice and Quasar shows the exact answer you keep choosing next to what's true, offers other angles, and waits for a correct unaided answer before clearing it. Quasar Plus can write a brand-new memory hook aimed at that exact mistake (the blueprint's self-repairing mnemonic). | `src/lib/learning.ts`, `src/features/ErrorMemory.tsx` |
...
| **Notes → Quiz** | Paste notes or choose a .txt, .md or PDF file. You study first: your notes laid out by heading with the key terms highlighted, then flashcards, then the quiz. The free quiz is built on your device: it reads headings, bullets, numbered steps, "Term: definition" lines and sentences such as "Glycolysis takes place in the cytoplasm", and mixes definition, fill-the-gap, number, which-section and what-comes-next questions with wrong answers drawn from the same notes. Quasar Plus adds AI quizzes whose questions each quote the line they come from; for pasted text, questions that can't be traced back are dropped. Every question returns for spaced review. | `src/features/NotesQuiz.tsx`, `src/lib/noteQuiz.ts`, AI task `quiz` |
...
| RevenueCat SDK integrated | `react-native-purchases` with offerings, purchase, restore and `quasar_pro` entitlement checks in `src/lib/services.ts`. Plus unlocks personalized stories, AI tutor feedback, AI quizzes and new memory hooks, all behind one AI gateway (`supabase/functions/ai`) that checks RevenueCat on the server. On the web build, purchases go through RevenueCat Billing (Stripe); [docs/GO-LIVE.md](docs/GO-LIVE.md) walks through a sandbox purchase | Implemented |
